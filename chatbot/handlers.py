...
FLIGHT_CALENDAR, NUMBER_OF_SEATS_CALENDAR = range(12, 14)
...
def check_date_calendar(update, context, date):
    ...
    dates = getting_date(flight_date=context.user_data['date_of_departure'],
    ...
    update.callback_query.edit_message_text(text=f"Вы выбрали {context.user_data['date_of_departure']}\n"
...
def help_command(update, context):
    ...
    update.message.reply_text('Введите /start или /ticket, чтобы начать.\n Затем выполняй команды бота.\n '
...
def other_message(update, context):
    ...
    content_type = {'audio': update.message.audio, 'photo': update.message.photo, 'voice': update.message.voice,
                    'video': update.message.video, 'document': update.message.document,
                    'location': update.message.location, 'contact': update.message.contact,
...
def start(update, context):
    ...
    update.message.reply_text('Привет! Я бот. Я помогаю подобрать рейс. Для начала введите город отправления\n'
...
def check_arrival_city(update, context):
    ...
    if text == "Моего города нет":
        ...
        if connection_between_cities(context):
            ...
            log.info(f"Между городами {context.user_data['city_of_departure']} и {context.user_data['arrival_city']} "
                     f"нет рейсов!")
            update.message.reply_text(f"Между городами {context.user_data['city_of_departure']} и "
...
def date_of_departure(update, context):
    ...
    if date_flight > now:
        ...
        update.message.reply_text(
            "Ближайшие даты:",
            reply_markup=ReplyKeyboardMarkup(
                proposed, resize_keyboard=True, one_time_keyboard=True))
        return FLIGHT
    else:
...
def not_correct_date_of_departure(update, context):
    ...
    update.message.reply_text(f"Введена некорректная дата отправления {update.message.text}! "
...
def not_correct_phone_number(update, context):
    ...
    update.message.reply_text(f"Введен некорректный номер телефона {update.message.text}! "
...
def check_phone_number(update, context):
    ...
    if update.message.text == "да":
        ...
        update.message.reply_text("Спасибо! Ожидайте звонка.")
        return ConversationHandler.END
    else:
