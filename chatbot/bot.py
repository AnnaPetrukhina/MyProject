...
def main():
    ...
    # CITY_OF_DEPARTURE, ARRIVAL_CITY, DATE_OF_DEPARTURE, FLIGHT, NUMBER_OF_SEATS, COMMENT, PHONE_NUMBER
    ...
    conv_handler = ConversationHandler(
        ...
        states={
            handlers.CITY_OF_DEPARTURE: [MessageHandler(Filters.regex(re_city) & ~Filters.command,
                                                        handlers.city_of_departure)],
            handlers.CHECK_CITY_OF_DEPARTURE: [MessageHandler(Filters.text & ~Filters.command,
                                                              handlers.check_city_of_departure)],
            handlers.ARRIVAL_CITY: [MessageHandler(Filters.regex(re_city) & ~Filters.command,
                                                   handlers.arrival_city)],
            handlers. CHECK_ARRIVAL_CITY: [MessageHandler(Filters.text & ~Filters.command,
                                                          handlers.check_arrival_city)],
            handlers.METHOD_OF_DATE_SELECTION: [MessageHandler(Filters.regex("ввести самому") & ~Filters.command,
                                                               handlers.date_input),
                                                MessageHandler(Filters.regex("показать календарь"),
                                                               handlers.calendar_handler)],
            handlers.DATE_OF_DEPARTURE: [CallbackQueryHandler(handlers.inline_handler),
                                         MessageHandler(Filters.regex(re_date) & ~Filters.command,
            ...
            handlers.NUMBER_OF_SEATS: [MessageHandler(Filters.regex('1|2|3|4|5') & ~Filters.command,
                                                      handlers.number_of_seats),
                                       MessageHandler(Filters.text & ~Filters.command,
                                                      handlers.not_correct_number_of_seats)],
            ...
            handlers.COMMENT: [MessageHandler(Filters.regex('Пропустить|пропустить') & ~Filters.command,
                                              handlers.exit_comment),
                               MessageHandler(Filters.text & ~Filters.command, handlers.comment)],
            handlers.DATA_VALIDATION: [MessageHandler(Filters.regex("да|нет") & ~Filters.command,
                                                      handlers.data_validation)],
            handlers.PHONE_NUMBER: [MessageHandler(Filters.regex(re_phone) & ~Filters.command, handlers.phone_number),
                                    MessageHandler(Filters.text & ~Filters.command, handlers.not_correct_phone_number)],
            handlers.CHECK_PHONE_NUMBER: [MessageHandler(Filters.regex("да|нет") & ~Filters.command,
